...
const ABOUT_TEXT = "A frontend developer turning ideas into fast, accessible and polished interfaces. I work best with teams shipping products that need to feel as good as they look. Let's build something worth remembering.";
...
export default function MarqueeAbout() {
  useEffect(() => {
    ...
    if (row1 && row2) {
      row1.innerHTML = [...LOGOS.slice(0, 11), ...LOGOS.slice(0, 11), ...LOGOS.slice(0, 11)].map(tile).join('');
      row2.innerHTML = [...LOGOS.slice(11), ...LOGOS.slice(11), ...LOGOS.slice(11)].map(tile).join('');
    }
  }, []);
    ...
    <main className="flex-grow pt-24 relative">
      ...
      <section className="relative z-10 min-h-[80vh] flex flex-col items-center justify-center px-6 sm:px-8 md:px-12 py-20 overflow-hidden" id="about">
        <div className="flex flex-col items-center gap-16 sm:gap-20 md:gap-24 w-full max-w-5xl">
          ...
          <a
            className="group inline-flex items-center gap-2 rounded-full text-white font-medium uppercase tracking-[0.2em] px-8 py-4 text-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_40px_-10px_rgba(255,90,31,0.7)]"
            href="#contact"
            style={{
              background: 'linear-gradient(135deg, #ff8a4c 0%, #ff5a1f 48%, #c2410c 100%)',
              boxShadow: '0 8px 30px -8px rgba(255,90,31,0.55), inset 0 1px 0 rgba(255,255,255,0.35), inset 0 0 0 1px rgba(255,255,255,0.12)'
            }}
          >
            Contact Me
